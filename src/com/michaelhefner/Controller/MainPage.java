...
public class MainPage implements Initializable {
    ...
    public void deleteApp() throws SQLException {
        if (appointmentToBeModified != null) {
            ...
            Optional<ButtonType> result = alert.showAndWait();
            if (result.isPresent() && result.get() == ButtonType.OK) {
                Timeline.removeTimeSlot(new TimeSlot(appointmentToBeModified.getStart().toLocalDateTime(), appointmentToBeModified.getEnd().toLocalDateTime()));
...
//        });
    ...
    private void populateTimeline() {
        for (Appointment appointment : ScheduleEntries.getAllAppointments())
            Timeline.addTimeSlot(
                    new TimeSlot(appointment.getStart().toLocalDateTime(), appointment.getEnd().toLocalDateTime()));
    }
    ...
    private void populateAppointmentDataFromDB() throws SQLException {
        ...
        while (appointmentResultSet.next()) {
            ZonedDateTime start = appointmentResultSet.getTimestamp("start").toLocalDateTime().atZone(ZoneId.of("GMT+0"));
            ZonedDateTime end = appointmentResultSet.getTimestamp("end").toLocalDateTime().atZone(ZoneId.of("GMT+0"));
            ...
            checkAppTimeEminent(appointment.getStart().toLocalDateTime(), appointment.getTitle());
        }
        Connect.closeConnection();
    }
