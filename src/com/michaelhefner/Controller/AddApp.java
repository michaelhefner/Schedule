...
public class AddApp implements Initializable {
    ...
    private boolean setStartAndEndDateTimeAndValidate() {
        ...
        if (startAppointmentDateTime.isAfter(LocalDateTime.of(startAppointmentDateTime.getYear(),
                ...
                8, 0))
                && startAppointmentDateTime.isBefore(LocalDateTime.of(startAppointmentDateTime.getYear(),
                startAppointmentDateTime.getMonth(), startAppointmentDateTime.getDayOfMonth(),
                18, 0))
                && endAppointmentDateTime.isBefore(LocalDateTime.of(endAppointmentDateTime.getYear(),
                endAppointmentDateTime.getMonth(), endAppointmentDateTime.getDayOfMonth(),
                18, 0))
        ) {
            return true;
        } else {
    ...
    public void onSaveClicked() throws SQLException {
        ...
        if (isValid) isValid = validateDate();
        if (isValid) isValid = addTimeSlot();
        if (isValid && !isModifyAppointment && customerToAddToAppointment != null) {
            ...
            if (updateAppointment()) {
                ...
                Timeline.removeTimeSlot(new TimeSlot(startAppointmentDateTime, endAppointmentDateTime));
    ...
    private boolean addTimeSlot(){

        if(Timeline.addTimeSlot(new TimeSlot(startAppointmentDateTime, endAppointmentDateTime)))
            return true;
        else {
    ...
    private void addAppointment() throws SQLException {
        ...
        ZonedDateTime start = Instant.from(startAppointmentDateTime.atZone(ZoneId.systemDefault())).atZone(ZoneId.of("GMT+0"));
        ZonedDateTime end = Instant.from(endAppointmentDateTime.atZone(ZoneId.systemDefault())).atZone(ZoneId.of("GMT+0"));
        if (customerToAddToAppointment != null) {
            appointment = new Appointment(customerToAddToAppointment.getId(),
                    ...
                    txtType.getText(), txtUrl.getText(), start.withZoneSameInstant(ZoneId.systemDefault()),
            ...
            hashMap.put(9, start.toLocalDate() + " " + start.toLocalTime());
            hashMap.put(10, end.toLocalDate() + " " + end.toLocalTime());
    ...
    private boolean updateAppointment() throws SQLException {
        ...
        appointmentToModify.setStart(start.withZoneSameInstant(ZoneId.systemDefault()));
        appointmentToModify.setEnd(end.withZoneSameInstant(ZoneId.systemDefault()));
        ...
        hashMap.put(8, start.toLocalDate() + " " + start.toLocalTime());
        hashMap.put(9, end.toLocalDate() + " " + end.toLocalTime());
    ...
    public void isModify(Appointment appointment) {
        TimeSlot tempSlot = Timeline.getDateTimeObservableList().filtered(timeSlot ->
                timeSlot.getStart().isEqual(appointment.getStart().toLocalDateTime())).get(0);
        ...
        setAMPMFormat(appointment.getStart().toLocalDateTime(), appointment.getEnd().toLocalDateTime());
    }
    ...
    private boolean isStartBeforeEndTime(LocalDateTime start, LocalDateTime end) {
